...
class VLiteDatabase(DatabaseStore):
    def __init__(
        self,
        *,
    ...
    def add_node(self, label: str, attribute: Dict, id: Any):
        ...
        attribute.update({"label": label, "type": "node", "id": id, "embed_id": count})
    ...
    def update_node(self, node: Node):
        node_embed_id = self.vlite.get(where={"id": node.id})["embed_id"]
        embed_id = {"embed_id": node_embed_id}
        self.vlite.update(
            node.id, text=node.label, metadata={**node.attributes, **embed_id}
        )
        self.vlite.save()
    ...
    def fetch_node_embed_id(self, node_id: Any):
        ids = ()
        nodes = self.vlite.get(where={"id": node_id})
        for node in nodes:
            ids += (node["embed_id"],)
        return ids
    ...
    def fetch_edge_embed_ids(self, id: Any):
        ids = ()
        sources = self.vlite.get(where={"source": id})
        targets = self.vlite.get(where={"target": id})
    ...
    def search_edge(self, source: Any, target: Any, attributes: Dict):
        # TODO: need to consider attributes
        edge = self.vlite.get(where={"source": source, "target": target})
        if edge is None:
            return None
        else:
            return edge["embed_id"]

    def fetch_node_id(self, id: Any):
        return self.vlite.get(where={"embed_id": id})["id"]

    def find_nodes_by_label(self, label: str):
        return self.vlite.get(where={"label": label})
    ...
    def search_similar_nodes(
        ...
        *,
        ...
        results = []
        for id in embed_ids:
            results.append(self.vlite.get(where={"embed_id": id})[0])

        if sort_by:
            results.sort(key=lambda x: x[2].get(sort_by, 0), reverse=desc)
