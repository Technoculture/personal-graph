...
def _insert_node(
    ...
    cursor.execute(
        ...
        (count, json.dumps(embedding_model.get_embedding(json.dumps(set_data)))),
    )
    connection.commit()
...
def vector_search_node(
    ...
    def _search_node(cursor, connection):
        embed = json.dumps(embedding_model.get_embedding(json.dumps(data)))
...
def add_node(
    ...
    def _add_node(cursor, connection):
        _insert_node(cursor, connection, embedding_model, identifier, label, data)
        ...
        for i, x in enumerate(zip(ids, labels, nodes)):
            ...
            cursor.execute(
                ...
                (
                    count + i,
                    json.dumps(
                        embedding_model.get_embedding(json.dumps(_set_id(x[0], x[2])))
                    ),
                ),
            )
...
def _upsert_node(
    ...
    if not current_data:
        ...
        cursor.execute(
            ...
            (
                count,
                json.dumps(embedding_model.get_embedding(json.dumps(updated_data))),
            ),
        )
        cursor.execute(
...
def upsert_node(
    embedding_model: OpenAIEmbeddingsModel, identifier: Any, label: str, data: Dict
) -> CursorExecFunction:
    def _upsert(cursor, connection):
        _upsert_node(cursor, connection, embedding_model, identifier, label, data)
...
def upsert_nodes(
    ...
    def _upsert(cursor, connection):
        for id, label, node in zip(ids, labels, nodes):
            _upsert_node(cursor, connection, embedding_model, id, label, node)
...
def connect_nodes(
    ...
    def _connect_nodes(cursor, connection):
        ...
        cursor.execute(
            ...
            (count, json.dumps(embedding_model.get_embedding(json.dumps(edge_data)))),
        )
        connection.commit()
...
def pruning(
    embedding_model: OpenAIEmbeddingsModel, threshold: float
) -> CursorExecFunction:
    def _merge(cursor, connection):
        ...
        for node_id in nodes:
            ...
            similar_nodes = vector_search_node(
                embedding_model, node_data, threshold, 3
            )(cursor, connection)
                ...
                for data in in_degree:
                    ...
                    cursor.execute(
                        ...
                        (
                            count,
                            json.dumps(
                                embedding_model.get_embedding(json.dumps(edge_data))
                            ),
                        ),
                    )
                    connection.commit()
...
def find_similar_nodes(
    ...
    def _identical_nodes(cursor, connection):
        ...
        similar_rows = []
        for node in nodes:
            similar_nodes = vector_search_node(embedding_model, node, threshold, 2)(
                cursor, connection
            )
