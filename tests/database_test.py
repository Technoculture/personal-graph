...
def test_bulk_operations(database_test_file, nodes, edges):
    ...
    db.atomic(database_test_file, db.connect_many_nodes(sources, targets, properties))
    for src, tgts in edges.items():
        actual = [
            tuple(x)
            for x in [
                [edge[0], edge[1], json.loads(edge[2])]
                for edge in db.atomic(
...
def test_exception(database_test_file, apple, nodes):
    ...
    ) as e:  # should be thrown since we are inserting a duplicate node
...
def test_search(database_test_file, apple, nodes):
    ...
    steves = db.atomic(database_test_file, db.find_nodes([kv_name_like], ("Steve%",)))
    assert len(steves) == 2
    ...
    assert list(map(lambda x: x["name"], woz_mike)) == [
        "Steve Wozniak",
        "Mike Markkula",
    ]
    ...
    founders = db.atomic(
        database_test_file,
        db.find_nodes(
            [db._generate_clause("type", tree=True)],
            ("founder",),
            tree_query=True,
            key="type",
        ),
    ...
    assert list(map(lambda x: x["name"], founders)) == [
        "Steve Wozniak",
        "Steve Jobs",
        "Ronald Wayne",
    ]
...
def test_traversal(database_test_file, apple):
    ...
    assert db.traverse(database_test_file, "2", "3") == ["2", "1", "3", "4", "5"]
    ...
    assert db.traverse(
        database_test_file, 5, neighbors_fn=db.find_inbound_neighbors
    ) == ["5"]
    assert db.traverse(
        database_test_file, "5", neighbors_fn=db.find_inbound_neighbors
    ) == ["5"]
    assert db.traverse(
        database_test_file, 5, neighbors_fn=db.find_outbound_neighbors
    ) == ["5", "1", "4"]
    assert db.traverse(
        database_test_file, "5", neighbors_fn=db.find_outbound_neighbors
    ) == ["5", "1", "4"]
...
def test_traversal_with_bodies(database_test_file, apple):
    ...
    assert _normalize_results(
        db.traverse(database_test_file, 2, 3, with_bodies=True)
    ) == _normalize_results(
        [
            (
                "2",
    ...
    assert _normalize_results(
        db.traverse(
            database_test_file, 5, with_bodies=True, neighbors_fn=db.find_neighbors
        )
    ) == _normalize_results(
        [
            ("5", "()", '{"name":"Mike Markkula","type":["person","investor"],"id":5}'),
